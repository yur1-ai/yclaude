...
function makeMultiProviderEvents(): UnifiedEvent[] {
  return [
    ...
    makeEvent(0.03, { provider: 'cursor', sessionId: 'cursor-s1', timestamp: '2024-01-15T10:30:00Z', model: 'gpt-4o', cwd: '/projects/a', sessionType: 'composer' } as UnifiedEvent),
    makeEvent(0.04, { provider: 'cursor', sessionId: 'cursor-s1', timestamp: '2024-01-15T10:35:00Z', model: 'gpt-4o', cwd: '/projects/a', sessionType: 'composer' } as UnifiedEvent),
    makeEvent(0.02, { provider: 'cursor', sessionId: 'cursor-s2', timestamp: '2024-01-15T12:00:00Z', model: 'gpt-4o-mini', cwd: '/projects/c', sessionType: 'edit' } as UnifiedEvent),
  ];
}
...
describe('/api/v1/sessions — sessionType filtering', () => {
  it('returns only composer sessions (+ Claude sessions without sessionType) when ?sessionType=composer', async () => {
    ...
    const sessionIds = body.sessions.map((s: Record<string, unknown>) => s.sessionId);
...
describe('/api/v1/chats — provider filtering', () => {
  ...
  it('returns provider field on each chat row', async () => {
    ...
    const providers = body.chats.map((c: Record<string, unknown>) => c.provider);
...
describe('/api/v1/activity — provider filtering', () => {
  it('returns per-provider session counts when no ?provider= filter', async () => {
    ...
    const jan15 = body.data.find((d: Record<string, unknown>) => d.date === '2024-01-15');
...
describe('/api/v1/models — provider filtering', () => {
  it('returns provider field on each model row', async () => {
    ...
    const sonnet = body.rows.find((r: Record<string, unknown>) => r.model === 'claude-sonnet-4-20250514');
    expect(sonnet?.provider).toBe('claude');
    // gpt-4o has 2 events, all from cursor
    const gpt4o = body.rows.find((r: Record<string, unknown>) => r.model === 'gpt-4o');
  ...
  it('returns only claude model rows when ?provider=claude', async () => {
    ...
    const modelNames = body.rows.map((r: Record<string, unknown>) => r.model);
