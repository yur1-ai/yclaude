# Requirements Archive: yclaude v1.0

**Archived:** 2026-02-28 — v1.0 Local MVP (Phases 1–4) shipped
**v1.0 scope:** 10/20 v1 requirements shipped (Phases 1–4); 10 pending requirements move to v1.1
**Audit:** milestones/v1.0-MILESTONE-AUDIT.md — status: tech_debt (no blockers; 10/10 phase-scoped requirements satisfied)
...
Next milestone after v1.0. Priorities noted inline.

### Analytics Enhancements

- [ ] **ANLT-09** *(high priority)*: User can select a 24-hour window on the cost-over-time chart that renders hourly buckets — gives intraday spend visibility for heavy daily users; toggling to "24h" swaps the bucket granularity from daily to hourly and adjusts x-axis tick labels accordingly

### Distribution

- [ ] **DIST-01** *(high priority)*: Package is published to npm as `yclaude` via manual local publish — pre-built web assets bundled in the tarball, no source maps, correct `main`/`bin`/`files` fields, `.npmignore` excluding dev artifacts, semver tagging, and a polished README with install instructions and badge
- [ ] **DIST-02** *(high priority)*: Automated CI/CD pipeline via GitHub Actions publishes to npm on git tag push — runs lint, typecheck, test, build, then `npm publish`; eliminates manual local publishing for all future releases

### Conversations Viewer

- [ ] **CHAT-01** *(low priority)*: User can view conversation content in a dedicated Chats tab (or sub-tab under Sessions) with explicit opt-in — requires `--show-messages` flag at startup to enable; no conversation text displayed by default; `applyPrivacyFilter()` becomes conditional on the flag; aligns with MSGS-01–04 intent
...
| ANLT-09 | v1.1 — Phase TBD (chart enhancement) | Planned |
| DIST-01 | v1.1 — Phase TBD (distribution) | Planned |
| DIST-02 | v1.1 — Phase TBD (distribution) | Planned |
| CHAT-01 | v1.1 — Phase TBD (conversations viewer) | Planned (low priority) |

**Coverage:**
- v1 requirements: 20 total
- Mapped to phases: 20
- Unmapped: 0
- v1.1 requirements planned: 4 (ANLT-09, DIST-01, DIST-02, CHAT-01)

---
*Requirements defined: 2026-02-28*
*Last updated: 2026-02-28 — Added v1.1 requirements: ANLT-09 (24h chart window), DIST-01/DIST-02 (npm distribution + CI), CHAT-01 (conversations viewer)*
